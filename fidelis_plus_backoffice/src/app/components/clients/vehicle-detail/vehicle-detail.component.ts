      ...
      <header class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div class="space-y-4">
          <nav class="flex items-center gap-2 text-[10px] font-bold text-outline/40 uppercase tracking-widest">
            <a routerLink="/clients" class="hover:text-primary transition-colors">Clientèle</a>
            <span class="material-symbols-outlined text-[10px]">chevron_right</span>
            <a [routerLink]="['/clients', vehicle().company_id]" class="hover:text-primary transition-colors">Détails Client</a>
          </nav>

          <a *ngIf="requestId()" [routerLink]="['/vente']" [queryParams]="{ tab: 'requests', request_id: requestId() }"
          ...
          <div class="flex flex-col md:flex-row md:items-center gap-4 md:gap-8">
            <h1 class="text-4xl md:text-6xl font-headline font-black text-on-surface tracking-tighter">
              {{ vehicle().brand }} <span class="text-outline/20 font-medium">{{ vehicle().model }}</span>
            </h1>
        ...
        <div class="flex gap-3">
          ...
          <a [routerLink]="['/clients', vehicle().company_id, 'vehicules', vehicle().id, 'editer']"
             title="Modifier le véhicule"
             class="w-12 h-12 rounded-2xl bg-surface-container border border-outline-variant/10 text-outline hover:text-primary transition-colors flex items-center justify-center">
            ...
            <div class="pt-8 border-t border-outline-variant/10">
              <div class="flex items-center gap-4 p-6 rounded-3xl bg-surface-container text-on-surface">
                 ...
                 <p class="text-[11px] font-medium leading-relaxed italic text-outline">
                    Le statut de conformité et la prochaine visite sont calculés automatiquement : véhicules de transport tous les 6 mois, véhicules personnels tous les 12 mois. Des rappels sont envoyés à J-14, J-0 et J+7 au commercial et au correspondant principal.
                 </p>
              </div>
            </div>
...
export class VehicleDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (rid) {
      const id = +rid;
      if (Number.isFinite(id) && id > 0) this.requestId.set(id);
    }
    this.refreshData();
  }
  ...
  refreshData(): void {
    ...
    if (vId) {
      this.vehicleService.getById(+vId).subscribe({
        next: (data) => this.vehicle.set(data),
        error: () => this.toastService.error('Échec du chargement.')
      });
    }
  }
